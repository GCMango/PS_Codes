...
int N, C, arr[MXN], cost[MXC], dp[MXN][MXC][1 << MXN];
...
int dfs(int n, int c, int chk) {
    ...
    for (int i = 0; i < N; ++i) {
        if (!(chk & (1 << i))) {
            ...
            //ret = max({ ret, dfs(i, sost, ichk) + cost[nost], dfs(i, arr[i], ichk) + cost[c]});
            ret = max({ ret, dfs(i, sost, ichk) + cost[nost], dfs(i, iost, ichk) + cost[sost], dfs(i, arr[i], ichk) + cost[c]});
            //ret = max({ ret, dfs(i, iost, ichk) + cost[sost], dfs(i, arr[i], ichk) + cost[c]});
        }
    }
    return ret;
}
